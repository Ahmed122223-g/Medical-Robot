...
class VoiceCommandProcessor:
    ...
    def __init__(self):
        ...
        print("✅ Voice Command Processor initialized (keyword mode)")
    ...
    def _get_system_prompt(self) -> str:
        return """
أنت مساعد ذكي لروبوت طبي. مهمتك فهم الأوامر الصوتية العربية وتحويلها لأوامر منظمة.

الأوامر المتاحة:
1. open_food - فتح كاميرا تحليل الطعام (كلمات: حلل الطعام، صور الأكل، افتح الكاميرا)
2. close_food - إغلاق الكاميرا
3. open_chat - بدء المحادثة (كلمات: تحدث معي، كلمني)
4. exit_app - إغلاق البرنامج (كلمات: اغلق البرنامج، خروج)
5. close_current - إغلاق/إيقاف الحالي (كلمات: اغلاق، توقف، كفاية)
6. general_chat - محادثة عامة (أي كلام آخر)
...
"""
    ...
    def set_current_screen(self, screen: str):
        ...
        if screen == "food":
            self.context.state = CommandState.FOOD_SCREEN
        elif screen == "chat":
            self.context.state = CommandState.CHAT_MODE
        else:
            if self.context.state in [CommandState.FOOD_SCREEN, CommandState.CHAT_MODE]:
    ...
    def process_command(self, text: str) -> Optional[str]:
        ...
        if self.context.state == CommandState.CHAT_MODE:
            return self._handle_chat(text)
        elif self.context.state == CommandState.FOOD_SCREEN:
            return self._handle_food_screen(text)
        else:
            return self._handle_new_command(text)
    
    def _handle_food_screen(self, text: str) -> str:
        text_lower = text.strip()
        
        if any(k in text_lower for k in ["حلل", "صور", "التقط", "تحليل", "خد صوره"]):
            if self.callbacks.get("capture_food"):
                self.callbacks["capture_food"]()
        ...
        if any(k in text_lower for k in ["اقفل التصوير", "اغلق الكاميرا", "اقفل الكاميرا", "اغلاق", "أغلق", "توقف", "كفاية", "رجوع"]):
            if self.callbacks.get("close_current"):
                self.callbacks["close_current"]()
            self.context.state = CommandState.IDLE
    ...
    def _parse_keywords(self, text: str) -> str:
        ...
        if any(k in text_lower for k in ["السكر", "قياس السكر", "مستوى السكر"]):
            ...
            if numbers:
                ...
                return self._execute_command({"command": "set_sugar", "parameters": {"value": sugar_value}, "response": f"تمام، سجلت قياس السكر {sugar_value}"})
            else:
                return self._execute_command({"command": "speak", "parameters": {"text": "قولي قيمة السكر يا غالي"}, "response": "قولي قيمة السكر يا غالي"})
        ...
        if self.current_screen == "qr":
            ...
            if numbers:
                sugar_value = numbers[0]
                return self._execute_command({"command": "set_sugar", "parameters": {"value": sugar_value}, "response": f"تمام، سجلت {sugar_value}"})
    ...
    def _execute_command(self, result: dict) -> str:
        ...
        if command == "open_food":
            if self.callbacks.get("open_food"):
                self.callbacks["open_food"]()
            return response or "سأفتح كاميرا تحليل الطعام"
        
        elif command == "close_food":
            if self.callbacks.get("close_food"):
                self.callbacks["close_food"]()
            return response or "تم إغلاق الكاميرا"
        
        elif command == "open_chat":
            if self.callbacks.get("open_chat"):
                self.callbacks["open_chat"]()
            self.context.state = CommandState.CHAT_MODE
            ...
            if self.callbacks.get("exit_app"):
                self.callbacks["exit_app"]()
            return response or "إلى اللقاء، أتمنى لك السلامة"
        
        elif command == "general_chat":
            text = params.get("text", "")
            if self.callbacks.get("general_chat"):
                return self.callbacks["general_chat"](text)
            return response
            
        elif command == "open_qr":
            if self.callbacks.get("open_qr"):
                self.callbacks["open_qr"]()
            return response

        elif command == "set_sugar":
            if self.callbacks.get("set_sugar"):
                self.callbacks["set_sugar"](params.get("value"))
            return response

        elif command == "generate_qr":
            if self.callbacks.get("generate_qr"):
                self.callbacks["generate_qr"]()
            return response

        elif command == "open_browser":
            if self.callbacks.get("open_browser"):
                self.callbacks["open_browser"]()
            return response

        elif command == "speak":
            if self.callbacks.get("speak"):
                self.callbacks["speak"](params.get("text"))
            return response
...
if __name__ == "__main__":
    print(voice_command_processor.process_command("تحدث معي"))
